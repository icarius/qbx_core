...
--- `job`, `gang`, `charinfo`, and `metadata` are stored as JSON in TEXT columns, so any
--- filter on them (searchPlayerEntities, fetchIsUnique) forces a full table scan with
--- per-row JSON parsing. Generated STORED columns materialize the specific paths that are
--- actually queried, so a normal index can cover them instead. Idempotent: safe to run on
--- every resource start.
...
local function upsertPlayerEntity(request)
    ...
    if exists then
        MySQL.update.await('UPDATE players SET userId = :userId, name = :name, money = :money, charinfo = :charinfo, job = :job, gang = :gang, position = :position, metadata = :metadata, last_logged_out = :last_logged_out WHERE citizenid = :citizenid', params)
    else
        MySQL.insert.await('INSERT INTO players (userId, citizenid, cid, license, name, money, charinfo, job, gang, position, metadata, last_logged_out) VALUES (:userId, :citizenid, :cid, :license, :name, :money, :charinfo, :job, :gang, :position, :metadata, :last_logged_out)', params)
    end
end
