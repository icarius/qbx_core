...
local function giveStarterItems(source)
    ...
    -- Bounded: ox_inventory creates the inventory almost immediately after load, but an
    -- unbounded poll here would hang this thread forever if that never happens (resource
    -- restart mid-load, ox_inventory error, etc.) instead of just skipping starter items.
    local attempts = 0
    while not exports.ox_inventory:GetInventory(source) do
        attempts += 1
        if attempts > 50 then
            lib.print.warn(('gave up waiting for ox_inventory to create an inventory for source %s'):format(source))
            return
        end
        Wait(100)
    end
...
lib.callback.register('qbx_core:server:getPreviewPedData', function(source, citizenId)
    ...
    -- Restrict preview ped data to characters owned by the requesting license, otherwise any
    -- connected client could read another account's skin/model by guessing a citizenid.
