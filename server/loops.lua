...
local function removeHungerAndThirst(src, player)
    ...
    -- skipSave on both: each SetMetadata call would otherwise trigger its own full
    -- PlayerData broadcast and DB save, so two calls per tick meant 2 broadcasts + 2 saves
    -- before the explicit Save() below even ran (3 saves total, per player, per interval).
    -- Batched here into a single broadcast + save for the pair.
    SetMetadata(src, 'thirst', newThirst, true)
    SetMetadata(src, 'hunger', newHunger, true)
