...
if isServer then
    ...
    function KickWithReason(source, reason, setKickReason, deferrals) -- luacheck: ignore
        ...
        CreateThread(function()
            ...
            -- Retries DropPlayer for up to 5 seconds in case the initial drop doesn't take
            -- (e.g. client mid-connect). Was previously a `while true do ... end` with no
            -- unconditional Wait() when `source` was falsy, which could hang the resource
            -- thread outright; it also spawned a stacking CreateThread per retry instead of
            -- just calling DropPlayer again, and broke out on the wrong ping condition.
            for _ = 1, 5 do
                DropPlayer(source --[[@as string]], reason)
                Wait(1000)
                if GetPlayerPing(source --[[@as string]]) < 0 then return end -- player disconnected
            end
        end)
    end
