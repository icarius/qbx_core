    ...
    function qbx.spawnVehicle(params)
        ...
        while attempts < 3 do
            ...
            if not pcall(function()
                ...
                -- Retry instead of erroring outright: on populated servers the owner-transfer
                -- handshake can legitimately take longer than 5s, and previously this branch
                -- aborted the whole spawn (no vehicle, no keys) rather than trying again like
                -- the plate-mismatch branch below already does.
                DeleteEntity(veh)
                attempts += 1
                goto continue
            end
